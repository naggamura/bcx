...
store_var (int var_name)
{
  ...
  if (var_name > 3)
  {
      ...
      else
	{
	  ...
	  toobig = !bc_num2long (ex_stack->s_num, &temp);
	}
      switch (var_name)
	{
	case 0:
...
store_array (int var_name)
{
  ...
  if(!bc_num2long (ex_stack->s_next->s_num, &idx) || idx < 0 || idx >= BC_DIM_MAX)
  {
...
//     (idx == 0 && !bc_is_zero(ex_stack->s_num))) 
  if(!bc_num2long (ex_stack->s_num, &idx) || idx < 0 || idx >= BC_DIM_MAX)
  {
    rt_error ("Array %s subscript out of bounds.", a_names[var_name]);
  }
  else
