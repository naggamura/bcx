  ...
  while (pc.pc_addr < functions[pc.pc_func].f_code_size
	...
	case 'L':  /* Length function. */
	  ...
	  if (ex_stack->s_num->n_len == 1 &&
	    ...
	    bc_int2num (&ex_stack->s_num, ex_stack->s_num->n_dec_scale);
	  else
	  */
		bc_int2num(&ex_stack->s_num, bc_dec_len(ex_stack->s_num));
	  break;

	case 'M':  /* Factorial function. */
		{
			if(0 != bc_raisemod (ex_stack->s_next->s_next->s_num, ex_stack->s_next->s_num, ex_stack->s_num, &ex_stack->s_next->s_next->s_num, scale))
				rt_error ("powmod(a,b,r): Invalid parameters");
			pop();
			pop();
		}
		break;

	case 'F':  /* Factorial function. */
		{
			if(!bc_factorial (&ex_stack->s_num))
				rt_error ("F(n): n not too big non-negative integer");
		}
		break;

	case 'P':  /* Permutation function. */
		{
			if(!bc_permutation (&ex_stack->s_next->s_num, &ex_stack->s_num))
				rt_error ("P(n,r): n, r not too big non-negative integers, n >= r");
			pop();
		}
		break;

	case 'C':  /* Combination function. */
		{
			if(!bc_combination (&ex_stack->s_next->s_num, &ex_stack->s_num))
				rt_error ("C(n,r): n, r not too big non-negative integers, n >= r");
			pop();
		}
		break;
	...
	{
		//ex_stack->s_num->n_sign = -ex_stack->s_num->n_sign;
		bc_num	n = bc_dup_num(ex_stack->s_num);
		n->n_sign = -n->n_sign;
	...
	c_code = !!!(ex_stack->s_num->n_sign);//bc_is_zero (ex_stack->s_num);
	assign (c_code);
	break;
...
push_b10_const (program_counter *progctr)
{
  ...
  len_t kdigits, kscale, leading_zeros;
  ...
  if (kdigits == 1 && kscale == 0)
	...
	{
		len_t	i, ql, rl, dig_len, sca_len;
		int64_t	dig;

		dig_len = (kdigits+LOG10_RADIX-1)/LOG10_RADIX;
		sca_len = (kscale+LOG10_RADIX-1)/LOG10_RADIX;

		build = bc_new_num(dig_len, sca_len);
		build->n_len = dig_len;
		build->n_dec_scale = kscale;	// important
		build->n_sign = 1;

		i = build->n_scale + dig_len - 1;

		ql = kdigits / LOG10_RADIX;
		rl = kdigits % LOG10_RADIX;
		if(rl > 0)
		{
			dig = 0;
			while(rl-- > 0)
			{
				dig = dig*10 + byte(progctr);
			}
			build->n_value[i--] = dig;
		}
		while(ql-- > 0)
		...
		{
			byte(progctr);
			ql = kscale / LOG10_RADIX;
			rl = kscale % LOG10_RADIX;
			while(ql-- > 0)
			{
				int		j;
